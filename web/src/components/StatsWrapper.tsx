...
const AverageBlock = ({title, current, moreThen, maxPercent, lang}: { title: string, current: string, moreThen: string, maxPercent: string, lang: 'ru' | 'en' }) => {
    return <div className={'flex flex-row rounded-xl w-[306px] bg-[#292929] pl-5 h-[89px] overflow-hidden'}>
        <div className={'grow py-3'}>
            ...
            <div className="flex flex-row items-center gap-4">
                ...
                <div className={'font-medium opacity-50 flex flex-row items-center gap-1'}>
                    <span className={'text-[32px] pb-[3px]'}>{'>'}</span>
        ...
        <div className='bg-[#1C4B43] flex flex-col items-center justify-center px-1 w-[47px] relative percent-block'>
            <div className={`bg-[#13FFD7] absolute left-0 bottom-0 w-full z-0`} style={{
                height: `${Math.min(100, Number(maxPercent))}%`
            }}/>
            <span className={'text-white font-semibold relative'}>{maxPercent}%</span>
...
const StatsWrapper: React.FC<StatsProps> = ({
    ...
    return (
        <div className="p-11 bg-white shadow-md rounded-lg max-w-[1007px] stats-wrapper relative text-white">
            <img src={Background} alt="stats background" className={'w-full h-full absolute top-0 left-0 z-0'}/>
            <div className={'z-10 w-full h-full relative shadow-custom px-[54px] py-[40px] content-box flex flex-row justify-between max-w-[917px]'}>
                ...
                <div className={'flex flex-col basis-[337px] shrink-0'}>
                    ...
                    <div className={'flex flex-col gap-3'}>
                        <AverageBlock lang={lang} title={lang === 'ru' ? text9_ru : text9_en} current={`${dailyDeals.toFixed(0)}`} moreThen={dailyTradesMoreThen.toFixed(0)} maxPercent={dailyTradesMorePercent.toFixed(0)}/>
                        <AverageBlock lang={lang} title={lang === 'ru' ? text10_ru : text10_en} current={`${dailyDeals.toFixed(0)}`} moreThen={dailyProfitMoreThen.toFixed(0)} maxPercent={dailyProfitMorePercent.toFixed(0)}/>
                    </div>
                </div>
            </div>
        </div>
